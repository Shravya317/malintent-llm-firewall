...
def test_sha256_hash_cannot_reconstruct_prompt() -> None:
    ...
    ), "Original text found inside its own SHA-256 hash"
    ...
    print(
        f"✓ TEST 2 PASSED — hash: {stored_hash[:16]}... (original cannot be recovered)"
    )
...
def test_permission_validator_blocks_by_role() -> None:
    ...
    assert (
        v.check("customer", "logs:export") is False
    ), "Customer should not export logs"
    assert (
        v.check("customer", "config:read") is False
    ), "Customer should not read config"
    assert (
        v.check("customer", "config:write") is False
    ), "Customer should not write config"
    assert (
        v.check("customer", "db:users") is False
    ), "Customer should not access DB users"
    assert (
        v.check("customer", "db:accounts") is False
    ), "Customer should not access DB accounts"
    ...
    assert (
        v.check("employee", "config:write") is False
    ), "Employee should not write config"
    ...
    assert (
        v.check("Admin", "config:write") is True
...
def test_tool_access_controller_enforces_whitelist() -> None:
    ...
    assert (
        "users" in d.denial_reason
    ), f"Denial reason should mention 'users': {d.denial_reason}"
    ...
    assert (
        d_denied.denial_reason is not None and len(d_denied.denial_reason) > 0
