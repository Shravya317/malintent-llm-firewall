...
class PatternEngineResult:
    ...
    def to_dict(self) -> dict:  # ← 4 spaces indented inside the class
        return {
            ...
            "matched_pattern_ids": [m.pattern_id for m in self.matches],
        }
...
class PatternEngine:
    ...
    DIRECT_INJECTION_PATTERNS = [
        ...
        (
            "DI-002",
            r"\b(disregard|forget|override|bypass|skip)\b.{0,30}\b(instructions?|prompt|rules?|guidelines?|constraints?)\b",
        ),
        (
            "DI-003",
            r"\b(do not|don't|stop)\b.{0,20}\b(follow|obey|adhere to)\b.{0,30}\b(instructions?|rules?|guidelines?)\b",
        ),
        ("DI-004", r"your (new|actual|real|only) instructions?\b"),
        ("DI-005", r"\bnew (task|objective|goal|mission|directive|instruction)\b"),
        (
            "DI-006",
            r"(instead|rather).{0,30}(do|say|respond|output|write).{0,30}(following|this)",
        ),
        ("DI-007", r"from now on.{0,40}(you (are|will|must|should)|always|never)"),
        (
            "DI-008",
            r"(?i)(---+\s*end|end\s*of\s*(prompt|instructions)|</instructions>|</?new_instructions>)",
        ),
    ...
    PERSONA_OVERRIDE_PATTERNS = [
        (
            "PO-001",
            r"\b(you are|you're|act as|pretend (you are|to be)|roleplay as|imagine you are)\b.{0,50}\b(unrestricted|without (limits?|restrictions?|filters?|constraints?))\b",
        ),
        ("PO-002", r"\bDAN\b"),
        ...
        (
            "PO-003",
            r"\b(jailbroken?|jailbreak(ed)?|uncensored|unfiltered|unrestricted) (AI|model|version|mode)\b",
        ),
        (
            "PO-004",
            r"(pretend|imagine|roleplay|act).{0,40}(no (restrictions?|rules?|limits?|guidelines?)|without (rules?|restrictions?|limits?))",
        ),
        ("PO-005", r"\bact as (an? )?(evil|malicious|unrestricted|unethical)\b"),
        (
            "PO-006",
            r"you (have no|don't have any|are free from) (restrictions?|constraints?|limitations?|rules?|ethics?)",
        ),
        (
            "PO-007",
            r"(switch|change|toggle).{0,20}(mode|persona|character|identity).{0,30}(developer|admin|unrestricted|god)",
        ),
    ]
    ...
    DATA_EXFILTRATION_PATTERNS = [
        (
            "DE-001",
            r"(repeat|print|output|display|show|reveal|tell me|share).{0,40}(system prompt|initial prompt|original instructions?|configuration)",
        ),
        (
            "DE-002",
            r"what (are|were) (you|your) (instructions?|told|trained|programmed|prompted)",
        ),
        (
            "DE-003",
            r"(leak|expose|dump|extract|exfiltrate).{0,40}(data|information|prompt|context|memory|conversation)",
        ),
        (
            "DE-004",
            r"(show|display|output|print).{0,20}(all|every|full|complete|entire).{0,20}(conversation|history|context|memory|logs?)",
        ),
        ...
        (
            "DE-005",
            r"(reveal|show|share|output|print).{0,30}(api key|secret key|password|credentials?|access token)"
            r"|"
            r"(api key|secret key|password|credentials?|access token).{0,30}(reveal|show|share|output|print)",
        ),
        (
            "DE-006",
            r"what (is|was) (the )?(first|initial|original) (message|prompt|instruction|system)",
        ),
    ]
    ...
    ENCODING_PATTERNS = [
        ...
        (
            "EO-004",
            r"(decode|base64|hex|rot13|cipher).{0,30}(this|following|below|message)",
        ),
    ...
    INDIRECT_INJECTION_PATTERNS = [
        (
            "II-001",
            r"\[?(SYSTEM|ADMIN|ROOT|OVERRIDE)\]?\s*:?\s*(override|ignore|new instruction)",
        ),
    ...
    CONTEXT_MANIPULATION_PATTERNS = [
        (
            "CM-001",
            r"(above|previous|prior).{0,20}(text|content|message|context).{0,20}(was|is) (fake|false|fictional|hypothetical|a test)",
        ),
        (
            "CM-002",
            r"(this is|we are).{0,30}(test|simulation|exercise|drill).{0,30}(ignore|disregard|bypass)",
        ),
        ("CM-003", r"(maintenance|debug|developer|admin) mode"),
        (
            "CM-004",
            r"(end of|closing) (prompt|instructions?|system).{0,30}(begin|start|new)",
        ),
        ("CM-005", r'(?i)\{\s*"role"\s*:'),
        ("CM-006", r"(?i)^(Human|Assistant|User|AI)\s*:"),
        (
            "CM-007",
            r"(?i)(hypothetical|fictional|alternate\s+universe|in\s+a\s+world\s+where|post-apocalyptic)",
        ),
        (
            "CM-008",
            r"(?i)(for\s+(educational|academic|research|training)\s+purposes|for\s+(a|my)\s+(paper|thesis|course|class))",
        ),
    ]
    ...
    HARMFUL_ELICITATION_PATTERNS = [
        (
            "HE-001",
            r"(step.by.step|detailed|exact).{0,30}(instructions?|guide|tutorial).{0,40}(harm|illegal|weapon|exploit|hack|bypass|crack)",
        ),
        (
            "HE-002",
            r"(for (educational|research|fictional|hypothetical) purposes?).{0,50}(how to|steps|guide).{0,30}(hack|exploit|bypass|illegal)",
        ),
        (
            "HE-003",
            r"(in a (story|novel|movie|game)|as a character).{0,50}(explain|describe|show).{0,30}(how to|steps for)",
        ),
        (
            "HE-004",
            r"(?i)\b(bomb|weapon|explosive|malware|ransomware|keylogger|poison|meth|fentanyl|ricin|nerve\s+agent)\b",
        ),
        (
            "HE-005",
            r"(?i)\b(kill|murder|torture|stalk|kidnap|trafficking|suicide|swatting|threaten)\b",
        ),
        (
            "HE-006",
            r"(?i)\b(fraud|counterfeit|forge|phishing|blackmail|extort|bribery|launder|shoplift|wiretap)\b",
        ),
        (
            "HE-007",
            r"(?i)\b(illegal(ly)?|unregistered|untraceable|undetected|without\s+getting\s+caught)\b",
        ),
    ]
        ...
        for category, patterns in self._CATEGORY_MAP.items():
            for pattern_id, pattern in patterns:
                try:
                    ...
                    raise ValueError(
                        f"Invalid regex for {pattern_id!r}: {exc}"
                    ) from exc
        return compiled
    ...
    def _check_base64_decoded(self, text: str) -> bool:
        ...
        for match in b64_pattern.finditer(text):
            try:
                ...
                if re.search(
                    r"ignore|instruction|override|system", decoded, re.IGNORECASE
                ):
                    return True
            except Exception:
                pass
    ...
    def scan(self, text: str) -> PatternEngineResult:
        ...
        if not text or not isinstance(text, str):
            return PatternEngineResult(
                is_threat=False, explanation="Empty or invalid input."
            )
        ...
        if matches:
            ...
            primary_cat = max(
                ...
                key=lambda c: (
                    cat_counts[c],
                    -_PRIORITY.index(c) if c in _PRIORITY else -99,
                ),
            )
